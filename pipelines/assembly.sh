...
help_message () {
	...
	echo ""
	echo "	--metaspades-only	only assemble with metaspades"
	echo "	--megahit-only		only assemble with megahit (much faster)"
	echo "";}
...
metaspades_assemble=true; megahit_assemble=true


# load in params
OPTS=`getopt -o ht:m:o:1:2: --long help,megahit-only,metaspades-only -- "$@"`
...
while true; do
	case "$1" in
		...
		--metaspades-only) megahit_assemble=false; shift 1;;
		--megahit-only) metaspades_assemble=false; shift 1;;
...
if [ "$megahit_assemble" = true ]; then
	...
	########################################################################################################
        announcement "ASSEMBLING REMAINDER READS WITH MEGAHIT"
