...
g<-ggplot() + scale_colour_manual(values=paultol[[length(levels(mfilt[,taxlevel]))]], name="metaBAT\nBins", limits=levels(mfilt[,taxlevel]) )
...
g<-g +
  ...
  scale_y_log10(limits=c(1, 1200)) + scale_x_continuous(limits=c(0.2, 0.9),breaks = seq(0,1,.1)) +
